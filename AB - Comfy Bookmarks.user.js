...
$(".sidebar").prepend("<div class='box'>\
                          ...
                          <div class='pad'>\
                              <input type='button' id='everybookmark' value='Load all bookmarks'>\
                          </div>\
                          </div>\
                      </div>");
    ...
    for ( $nextpage = 2; $nextpage <= $lastpagenum; $nextpage++ ){
        $.ajax({
            ...
            success:  function(data){
                $(data).find("#collage_table > tbody").each(function(){ $("#collage_table > tbody").append($(this).html()); });
